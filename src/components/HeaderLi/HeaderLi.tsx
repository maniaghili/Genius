          ...
          <div className="bg-white dark:bg-gray-900  drop_down absolute z-10  top-10  w-48 min-h-40 rounded-r-lg  group-hover:visible ">
              <ul className="w-full ">
                {categories?.map((category:any)=>
                  ...
                  <Link to={`/series/${category.name}`} className="w-full text-black mt-1 dark:text-gray-300 hover:text-blue-600  h-8 cursor-pointer flex items-center text-[13px] font-semibold mr-2 justify-between">
                    <p>{category.title}</p>
