     ...
     <div className="w-[95%] h-[75px] flex justify-between">
      <div className="w-[50%] flex">
        ...
        <div className='flex flex-col gap-0 ssm:hidden md:inline-block'>
          <p className="opacity-70 akademi-header dark:text-white font-semibold ">آکادمی</p>
       ...
       <Link to="/series" className='opacity-65 text-[13px] font-bold dark:text-gray-300'>همه ی دوره ها</Link>
      ...
      <div className="w-md-[30%] ">
        <div className='flex w-fit items-center h-full gap-4'> 
          ...
          <CircleButton>
          <Link to={'./userbasket'} className='parent bg-slate-100 dark:text-gray-200 dark:bg-gray-900 hover:bg-slate-200 transition-all '>
          ...
          <Link className='parent dark:bg-gray-900 dark:text-gray-200 bg-slate-100 hover:bg-slate-200 transition-all' to={navigateUser('/userprofile','/login')}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-5 h-5">
     ...
     </div>
     {
      ...
      <div  className='active_colaps dark:bg-transparent z-50 rounded-none absolute h-screen w-full lg:hidden flex justify-between'>
       ...
       <div className='z-50 w-full flex items-center justify-between'>
        ...
        <Link to={'/'} className='flex w-1/2 h-1/5  gap-3 items-center'>
       ...
       <div className='w-full flex  flex-col items-center'>
         <div className='flex items-center gap-2 mt-4 bg-slate-200 dark:bg-gray-900 rounded-2xl w-11/12 h-11 border border-neutral-700 border-opacity-30'>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 mr-2 cursor-pointer hover:text-blue-500">
          ...
          <input type="text" onBlur={(e:any)=>{
            if(e.target.value){
               Navigate(`/allCoursesSearch?search=${e.target.value}`)
            }
          }} className='bg-transparent h-full w-full' placeholder='دنبال چی میگردی؟'/>
         </div>
         
         <Link to={'/userBasket'} className='w-11/12 hover:text-blue-400 cursor-pointer mt-6 h-16 border-y-2 flex justify-between items-center dark:border-gray-600  border-y-neutral-200 '> 
        ...
        <div className='w-full h-40 '>
          <ul className='h-full '>
                  ...
                  <ul className='list-disc mr-10 mb-4'>
                    {
                      categories?.map((category : any)=>
                        <li className='hover:text-blue-500'><Link to={`/series/${category.name}`}>{category.title}</Link></li>
                      )
                    }
                  </ul>
                 </div>
            ...
            <Link to={'/series'} className='flex items-center gap-3'>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-5 h-5">
            ...
            <li className='opacity-60 flex justify-start items-center gap-3 h-10'>
               <Link to={navigateUser('/userProfile','/login')} className='flex items-center gap-3'>
               <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-5 h-5">
