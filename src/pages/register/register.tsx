...
const register:FC =memo(() => {
    ...
    const registerHandler = (e:userInfoType) => {
     const userInfo = {
      ...
      axios.post("https://academic.liara.run/v1/auth/register",userInfo,{
      }).then(res=>{
  ...
  <div className="h-screen flex items-start justify-center bg-background p-5">
      <div className="w-full max-w-sm space-y-5  rounded-2xl">
          <div className="bg-gradient-to-b from-secondary bg-gray-100 dark:text-gray-300 dark:bg-gray-900 to-background rounded-3xl space-y-5 px-5 pb-5">
              ...
              <form action="#" className="space-y-3" onSubmit={handleSubmit((e:userInfoType | any)=>registerHandler(e))}>
                  ...
                  <div className="flex items-start relative flex-col gap-3">
                      <input type="text" {...register('userName',{required:'نام کاربری را وارد کنید'})} placeholder="نام کاربری خود را وارد نمایید" dir="ltr" className="bg-gray-50 dark:bg-gray-800 dark:border-gray-700 border form-input w-full h-11 !ring-0 !ring-offset-0 bg-secondary border-border focus:border-border rounded-xl text-sm text-foreground placeholder:text-right px-5" />
          ...
          <div className="bg-gray-100 dark:bg-gray-900 dark:text-gray-300 rounded-xl space-y-5 p-5">
